...
public class IconLoader
{
    ...
    private static Dictionary<uint, (IconType Type, uint IconId, GCHandle ByteHandle)> _textureDictionary = new();
    ...
    public static uint GetIconTextureId(Icon16 icon)
    {
        ...
        var textureId = ImGuiRenderer.LoadTexture(gcHandle.AddrOfPinnedObject(), 16, 16);
        _textureDictionary[textureId] = (IconType.Icon16, (uint)icon, gcHandle);
        return textureId;
    }

    public static uint GetIconTextureId(Icon32 icon)
    {
        ...
        if (bytes == null)
            return 0;
        var gcHandle = GCHandle.Alloc(bytes, GCHandleType.Pinned);
        var textureId = ImGuiRenderer.LoadTexture(gcHandle.AddrOfPinnedObject(), 32, 32);
        _textureDictionary[textureId] = (IconType.Icon32, (uint)icon, gcHandle);
        return textureId;
    }
